...
**Table Code（牌桌码）**:
标识一张牌桌的 6 位字母数字串（不分大小写），设计成人能读出来、能口头报给别人。
自动生成的码避开 I/O/0/1 以免报错听错；`--code` 自己挑的不受这条限制。
...
**Position（位置）**:
一手牌里某个座位相对 Button 的位置，决定他在这条街上第几个说话。
标注用通行的缩写：BTN（庄家位）、SB、BB、UTG（第一个说话）、UTG+1…、LJ、HJ、CO（Button 右手第一位）。
单挑时 Button 自己就是小盲，标作 BTN/SB。
_Avoid_: 座位号, 顺位, 前位/中位/后位
