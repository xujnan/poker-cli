...
轮到你了    要跟 2
可以：fold / call（跟 2） / bet <4-195> / allin（推 195）
>
```

**正在动的那一帧**永远只讲正在打的这手牌（ADR-0017）；打完的那些不会消失，
而是作为普通输出留在上面，交给终端的回滚缓冲——想往回翻多远都行。
座位表上 `行动中…` 标着牌桌此刻在等谁：这是服务端广播的 `turn` 事件说的，
不是客户端自己按座位顺序推的（弃牌、全下、暂离都会让顺序拐弯，推错了屏幕就在骗人）。
