...
`fold`、`allin`、`bet` 另有单字母写法 `f`、`a`、`b 100`。
想歇会儿就 `sitout`（缩写 `so`，这手打完生效，座位和筹码都留着），回来敲 `sitin`（`si`），输光了先 `topup 200`（`t 200`）。

`bet` 永远是「把本轮总投入推到这个数」，不是「再加这么多」（ADR-0005）。没有 `raise`。
`check` 和 `call` 没有单字母写法：它们是唯一一对看着像的动作，都以 c 开头，
认错的代价是本该过牌却跟了注。`sitout` / `sitin` 同理，`s` 谁也没拿到。
单字母只在终端里认——JSON 线路上每个动作仍然只有一种拼法，写 agent 的人不必知道它们存在。
...
**正在动的那一帧**永远只讲正在打的这手牌（ADR-0017）。牌桌此刻在等谁有两个落点：
座位表上那一行的 `行动中…`，和流水最后一行——在等的时候眼睛盯的就是那儿，
下一条记录会从那里冒出来。后面那个数是他还剩多少时间（开桌时的 `--timeout`），
走到 0 服务端就替他做决定。轮到自己时座位表上换成 `▶`，同一行不重复说第二遍；
自己那份不画秒表，改由「轮到你了」那行静态地写上限时——这一屏在你打字的时候不重画，
一个停着不走的秒表比没有秒表更骗人。

这件事只信服务端广播的 `turn` 事件，客户端不自己按座位顺序推：
弃牌、全下、暂离都会让顺序拐弯，推错了屏幕就在骗人。

秒针每秒走一格，但它只重写真的变了的那几行，从不碰你正在敲的那一行——
所以「抢在别人行动时提前输入」现在是安全的。

屏幕最上面那一块是刚打完的那一手——一手打完到下一手只隔 `--hand-delay`，
不留的话「谁赢了这个底池」一闪就没了。整手都留着，画得下多少看屏幕多高：
三十行左右的终端能把一手牌画全，再矮就从开头裁，标题里会写「略去开头 N 行」。
只留上一手，再往前的不留——留着就又变回「往上翻三屏找自己的底牌」。
...
断言屏幕没有越打越长、正在打的那一手只有一份；滚动那条一个转义序列都不许漏出去。
显式写 `--format=text` 或 `--format=live` 就按你说的办。

输入行交给终端自己的行编辑，没有历史也没有补全——要的不是一个全屏 TUI，
是「一屏状态 + 一行输入」。
...
- `internal/server/topup_test.go` 里的破产那两条 —— 让浅筹码那个人**只弃牌**，
  「他会输光」于是从碰运气变成必然。两个策略一样的机器人对打，浅筹码那个人的筹码是个鞅，
  他可能先涨上去（实测冲到 45），于是输光在任何固定时限里都不保证发生——那条测试原本 17% 会挂。
  现在还钉着一条守卫：他只弃牌就不可能翻倍，翻了说明有人把大脑换回去了。
